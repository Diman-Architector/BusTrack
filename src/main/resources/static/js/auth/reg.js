...
let btnRegPerform = document.getElementById("btnRegPerform");//переменная для кнопки btnLogPerform из файла modalReg.html
btnRegPerform.addEventListener("click", function() {//прописываем слушатель на клик по кнопке Регистрация
    ...
    if (xhr.status == 200) {
    /*Уведомление об успешной регстрации*/
        ...
        toastBootstrap.show();
        modalReg.hide();
        location.href = xhr.responseUrl; //чтобы из скрипта перейти с одной страницы на другую при успешной регистрации не получилось(((
    } else {
        let toastLiveExample = document.getElementById('liveToast');
        let alertBox = document.getElementById('alertBox');
        let toastBootstrap = bootstrap.Toast.getOrCreateInstance(toastLiveExample);
